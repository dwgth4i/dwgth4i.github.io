...
![alt text](/assets/img/posts/9409320974238093.png)

![alt text](/assets/img/posts/982348917632489987.png)
...
![alt text](/assets/img/posts/91283654238921351823.png)
...
![alt text](/assets/img/posts/92378478123278123.png)

# Bind the interface

For the binding/connect part, you can read this amazing [post](https://itm4n.github.io/from-rpcview-to-petitpotam/) from @itm4n, I learned a lot from his Windows posts. First, we need the idl file for the interface, as this already documented by Microsoft in [here](https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-even/0d0bee9c-dac5-46d9-b19b-2087826c02db), we can straight up copy and use it, but there is a caveat. At the time I just started using it, there are crazy amount of errors and here are some of them:
...
- In the Full IDL I mentioned above, the _RPC_STRING was for ANSI, I didn't pay attention and it took so much time to realize lol, [here](https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-dtyp/94a16bb6-c610-4cb9-8db6-26f15f560061) is what we need.
...
![alt text](/assets/img/posts/781239876564032174.png)
...
First case: `Procmon` shows that the service will perform a CreateFile and I get `0xC0000034` code which mean `STATUS_OBJECT_NAME_NOT_FOUND` (we can check it [here](https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-erref/596a1078-e883-4972-9bbc-49e60bebca55)) since the file is not exist, this is interesting as now we know that it might try to open the file as LOCAL_SERVICE.  

![alt text](/assets/img/posts/9324723874897129327.png)
...
![alt text](/assets/img/posts/512389734398759345.png)

Third attempt: This time I used a custom eventlog and the method return 0 which mean success, the service initially check the file header (128 bytes) and then it does ReadFile  in 4 bytes at a time until the end of the file. ([Event Log File Format](https://learn.microsoft.com/en-us/windows/win32/eventlog/event-log-file-format))

![alt text](/assets/img/posts/739128492389433459.png)
...
![alt text](/assets/img/posts/712312398198232234.png)
...
![alt text](/assets/img/posts/7234988189239813278.png)
...
![alt text](/assets/img/posts/9234239848912389.png)

Well, that is what I thought, in the following day I try again with a bit different syntax in my AD lab at home, I got some great news, and here is the lab info:

- Attacker: 192.168.112.128
- DC: 192.168.112.143
- WS: 192.168.112.145

Running this in attacker vm:

```
ntlmrelayx.py -smb2support -t dc
```

![alt text](/assets/img/posts/8233671287312873.png)

In DC machine or any other domain machine, we can execute the poc (you can do other ways just make sure it doesn't reflect), but here is the weird part, I have to ran it multiple times until it worked, I didn't know what happen but we did it lol
